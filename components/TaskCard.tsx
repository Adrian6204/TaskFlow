  ...
  return (
    <div 
        ...
        className={`
            ...
            ${isDragging ? 'opacity-50 scale-95' : 'hover:-translate-y-1 hover:bg-slate-50 dark:hover:bg-slate-800/80'}
            ${isBlocked ? 'cursor-not-allowed opacity-70 grayscale' : 'cursor-grab active:cursor-grabbing'}
        `}
    >
      <div className="flex justify-between items-start gap-2 mb-2">
        <h3 className="font-semibold text-sm text-slate-700 dark:text-slate-200 leading-snug line-clamp-2">{task.title}</h3>
        <div className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-slate-100 dark:bg-slate-900/80 rounded-lg p-1 absolute top-2 right-2 backdrop-blur border border-slate-200 dark:border-transparent">
          <button 
            onClick={(e) => { e.stopPropagation(); onToggleTimer(task.id); }}
            className={`p-1 rounded hover:bg-white dark:hover:bg-white/10 ${isTracking ? 'text-red-500 dark:text-red-400' : 'text-slate-400'}`}
          >
             {isTracking ? <StopIcon className="w-3 h-3" /> : <PlayIcon className="w-3 h-3" />}
          </button>
          <button onClick={(e) => { e.stopPropagation(); onEditTask(task); }} className="p-1 text-slate-400 hover:text-amber-500 hover:bg-white dark:hover:bg-white/10 rounded">
            <PencilIcon className="w-3 h-3" />
          </button>
          {onDeleteTask && (
            <button onClick={(e) => { e.stopPropagation(); onDeleteTask(task.id); }} className="p-1 text-slate-400 hover:text-red-500 hover:bg-white dark:hover:bg-white/10 rounded">
      ...
      <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-100 dark:border-white/5">
        <div className="flex items-center gap-2">
          {employee ? (
            <img src={employee.avatarUrl} alt={employee.name} className="w-6 h-6 rounded-full border border-slate-200 dark:border-white/10" title={employee.name} />
          ) : (
            <div className="w-6 h-6 rounded-full border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-slate-700"></div>
          )}
          <span className={`text-[10px] font-medium ${isOverdue ? 'text-red-500 dark:text-red-400' : 'text-slate-500'}`}>
        ...
        <div className="flex items-center gap-3">
            ...
            {totalSubtasks > 0 && (
                <div className="flex items-center text-slate-400 dark:text-slate-500 gap-1">
